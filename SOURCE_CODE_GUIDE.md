...
- **`android/app/google-services.json`** — included so the Android build works out of
  the box. These are **client-side** identifiers (already present in
  `firebase_options.dart` and in any shipped APK); access is protected server-side by
  the Firestore/Storage **security rules** and **App Check**, not by keeping the file
  private.
